...
- **AI:** the chat widget calls `POST /api/ai/chat` on our backend. The backend agent talks to Amazon
  Bedrock. The browser never sees the Bedrock key.
...
- [7. AI and Amazon Bedrock](#7-ai-and-amazon-bedrock)
...
the model provider and never holds the provider key — see [§7](#7-ai-and-amazon-bedrock).
...
Opsly backend agent  (AiChatService + ToolRegistry)
   ...
   v
Amazon Bedrock  ->  one foundation model
...
- **No key, no SDK, no provider URL in the browser.** The Bedrock API key lives only in the backend
  environment (`AWS_BEDROCK_API_KEY`). `NEXT_PUBLIC_*` values are public, so never put a secret there.
- Changing the model or region is a backend-only change (`BEDROCK_MODEL_ID`, `AWS_REGION`) — the
